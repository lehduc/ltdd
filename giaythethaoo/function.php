...
function userExists($username){
	$query = "SELECT tenKH FROM giaythethao WHERE tenKH = ?";
	global $con;
	if($stmt = $con->prepare($query)){//chuan bi 
		$stmt->bind_param("s",$tenKH); // troi buoc lai thong so
		$stmt->execute();// thuc thi cau lenh
		$stmt->store_result();// tra ve gia tri cua cua hang
		$stmt->fetch(); // dua ve , tra ve gia tri
		if($stmt->num_rows == 1){
			$stmt->close();
			return true;
		}
...
*/
function getSalt(){
	global $random_salt_length; // tao bien su dung chung global
